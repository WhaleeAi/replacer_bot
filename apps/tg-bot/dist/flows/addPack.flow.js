...
function registerAddPackFlow(bot, options) {
    ...
    bot.on("message:text", async (ctx, next) => {
        ...
        try {
            const packId = await (0, shared_1.createUserPack)(options.databaseUrl, userId, state.name, parsed.groupIds);
            options.state.clearAddPackState(userId);
            options.logger.info({ userId, packId, packName: state.name, groupsCount: parsed.groupIds.length }, "Pack created");
            await ctx.reply([
...
//# sourceMappingURL=addPack.flow.js.map